...
impl BilibiliProvider {
    ...
    async fn initialize(&mut self) -> Result<(), HlsStreamError> {
        ...
        if let Some((quality, url)) = self.available_qualities.iter().next() {
            ...
            return Err(HlsStreamError::InitializationError("No available qualities found".to_string()));
        }
    ...
    async fn fetch_room_info(&self) -> Result<serde_json::Value, HlsStreamError> {
        ...
        let json: serde_json::Value = response.json().await.map_err(HlsStreamError::NetworkError)?;
        
        if json.get("code").and_then(|v| v.as_u64()) != Some(0) {
            return Err(HlsStreamError::ParseError("Room info request failed".to_string()));
        }
    ...
    async fn fetch_play_info(&self) -> Result<serde_json::Value, HlsStreamError> {
        ...
        let response = self.client
            .get(&url)
            .header("Cookie", &self.cookies)
            .header("Referer", &format!("https://live.bilibili.com/{}", self.room_id))
            .send()
            .await
            .map_err(HlsStreamError::NetworkError)?;
        ...
        if json.get("code").and_then(|v| v.as_u64()) != Some(0) {
            return Err(HlsStreamError::ParseError("Play info request failed".to_string()));
        }
    ...
    fn parse_qualities(&mut self, play_info: &serde_json::Value) -> Result<(), HlsStreamError> {
        let play_url_info = play_info.get("playurl_info")
        ...
        for stream in play_url_info {
            let format_info = stream.get("format")
                .and_then(|v| v.as_array())
                .ok_or_else(|| HlsStreamError::ParseError("Invalid format structure".to_string()))?;
                
            for format in format_info {
                if format.get("format_name").and_then(|v| v.as_str()) == Some("fmp4") {
                    let codec_info = format.get("codec")
                        .and_then(|v| v.as_array())
                        .ok_or_else(|| HlsStreamError::ParseError("Invalid codec structure".to_string()))?;
                        
                    for codec in codec_info {
                        let url_info = codec.get("url_info")
                            .and_then(|v| v.as_array())
                            .ok_or_else(|| HlsStreamError::ParseError("Invalid URL info structure".to_string()))?;
                            
                        for url_data in url_info {
                            let host = url_data.get("host").and_then(|v| v.as_str())
                                .ok_or_else(|| HlsStreamError::ParseError("No host found".to_string()))?;
                            let extra = url_data.get("extra").and_then(|v| v.as_str())
                                .ok_or_else(|| HlsStreamError::ParseError("No extra found".to_string()))?;
                                
                            let base_url = codec.get("base_url").and_then(|v| v.as_str())
                                .ok_or_else(|| HlsStreamError::ParseError("No base URL found".to_string()))?;
                                
                            let full_url = format!("https://{}{}{}", host, base_url, extra);
                            
                            // Use quality description or fallback to codec
                            let quality_desc = codec.get("current_qn").and_then(|v| v.as_u64())
                                .map(|qn| match qn {
                                    10000 => "原画",
                                    400 => "蓝光",
                                    250 => "超清",
                            ...
                            self.available_qualities.insert(quality_desc.to_string(), full_url);
        ...
        if self.available_qualities.is_empty() {
            return Err(HlsStreamError::ParseError("No supported stream formats found".to_string()));
        }
...
impl HlsProvider for BilibiliProvider {
    async fn fetch_playlist(&self) -> Result<Vec<HlsSegment>, HlsStreamError> {
        let playlist_url = self.current_playlist_url.as_ref()
        ...
        let content = response.text().await.map_err(HlsStreamError::NetworkError)?;
        ...
        match parse_playlist_res(content.as_bytes()) {
            Ok(Playlist::MediaPlaylist(playlist)) => {
                ...
                for (i, segment) in playlist.segments.iter().enumerate() {
                    ...
                    .with_program_date_time(segment.program_date_time.clone().map(|dt| dt.to_rfc3339()))
                    .with_byte_range(segment.byte_range.clone().and_then(|br| br.offset.map(|offset| (offset, br.length))));
            ...
            Ok(_) => Err(HlsStreamError::ParseError("Not a media playlist".to_string())),
            Err(e) => Err(HlsStreamError::ParseError(format!("M3U8 parse error: {}", e))),
        }
    }
    ...
    async fn change_quality(&mut self, quality: &str) -> Result<(), HlsStreamError> {
        if let Some(url) = self.available_qualities.get(quality) {
            ...
            Err(HlsStreamError::ParseError(format!("Quality '{}' not available", quality)))
        }
    }
